...
Generates comprehensive PROD fabric topology diagrams showing BFC, OneFabric, and ws-cor devices
...
DEVICE_COLORS = {
    'root': '#FFD966',           # ROOT DC (Darker Orange)
    ...
    'ws-cor': '#FFE6CC',         # ws-cor (Light Orange)
}

class ProdFabricTopologyGenerator:
    """Agent to generate PROD fabric topology diagrams"""
    
    def __init__(self, site: str, fabric: str):
        self.site = site  # e.g., "bjs11-11"
        self.fabric = fabric  # e.g., "ws-c1"
        self.az = self.extract_az(site)  # e.g., "bjs11"
        
        # Create output directory under /Users/anishkt
    ...
    def normalize_device_name(self, device_name: str) -> str:
        ...
        Examples:
            bjs11-51-ws-c1-b3 -> bjs11-51-ws-c1
            bjs20-20-ws-c1-b10 -> bjs20-20-ws-c1
        """
    ...
    def categorize_device(self, device_name: str, device_data: Dict) -> Dict:
        ...
        if normalized_name == f"{self.site}-{self.fabric}":
            category = 'root'
            role = 'ROOT DC'
        elif device_type == 'core' or '-ws-cor' in device_name or '-cor' in device_name:
            category = 'ws-cor'
            role = 'WS-COR'
    ...
    def discover_neighbors(self, yaml_data: Dict) -> List[Tuple[str, Dict]]:
        ...
        for neighbor_name, neighbor_data in yaml_data['neighbors'].items():
            ...
            # Include BFC, OneFabric, and ws-cor
            if neighbor_type in ['bfc', 'onefabric', 'core']:
    ...
    def build_topology(self) -> Dict:
        ...
        Follows the 17-step process from requirements
        """
        print("\n" + "="*80)
        print(f"  PROD Fabric Topology Generator")
        ...
        if not root_yaml:
            ...
            print(f"     Save to: {self.yaml_dir}/{root_device}.json")
            return None
        ...
        print(f"  ✓ Found {len(root_neighbors)} BFC/OneFabric/ws-cor neighbors")
        ...
        intra_az_neighbors = []
        for neighbor_name, neighbor_data in root_neighbors:
            if self.is_same_az(neighbor_name) and neighbor_data.get('type') != 'core':
                intra_az_neighbors.append(neighbor_name)
                print(f"  • {neighbor_name} (same AZ)")
        
        # Step 5: Fetch YAML for all discovered neighbors (skip ws-cor)
        print(f"\n[Step 5] Fetching YAML for all neighbors...")
        for device_name in devices_to_fetch:
            # Skip ws-cor devices (they don't have YAML files)
            normalized = self.normalize_device_name(device_name)
            if '-cor' in normalized or '-cor' in device_name:
                print(f"  ⊘ Skipping ws-cor device: {normalized} (no YAML needed)")
                continue
        ...
        # Step 6: For intra-AZ neighbors, get THEIR neighbors too
        print(f"\n[Step 6] Discovering intra-AZ neighbor connections...")
        for intra_device in intra_az_neighbors:
            ...
            if normalized in self.yaml_cache:
                ...
                print(f"  • {normalized}: {len(intra_neighbors)} neighbors")
                
                # Fetch their YAMLs too
                for neighbor_name, _ in intra_neighbors:
                    if neighbor_name not in devices_to_fetch:
                        devices_to_fetch.add(neighbor_name)
        ...
        # Step 9: Filter devices (BFC, OneFabric, and ws-cor)
        ...
        # Add ws-cor devices directly from ROOT neighbors (no YAML needed)
        if 'neighbors' in root_yaml:
            for neighbor_name, neighbor_data in root_yaml['neighbors'].items():
                if neighbor_data.get('type') == 'core':
                    normalized = self.normalize_device_name(neighbor_name)
                    cat_data = self.categorize_device(neighbor_name, neighbor_data)
                    filtered_devices[normalized] = cat_data
                    print(f"  • Added ws-cor device: {normalized}")
        ...
        for device_name, yaml_data in self.yaml_cache.items():
            ...
            # Include BFC and OneFabric (ws-cor already added above)
    ...
    def generate_drawio_xml(self, topology_data: Dict) -> str:
        ...
        mxfile = ET.Element('mxfile', {
            ...
            'agent': 'PROD Fabric Topology Generator',
            'version': '24.0.0',
            'type': 'device'
        })
        
        diagram = ET.SubElement(mxfile, 'diagram', {
            'id': 'prod-fabric-topology',
        ...
        for category in ['root', 'ws-cor', 'bfc-inter-az', 'bfc-intra-az', 'onefabric']:
    ...
    def run(self):
        """Main execution flow"""
        print("\n" + "="*80)
        print("  PROD Fabric Topology Generator")
        print("="*80)
        ...
        # Summary
        print("\n" + "="*80)
        print("✅ PROD Fabric Topology Generation Complete!")
        print("="*80)
        print(f"\nOutput: {output_file}")
        ...
        print("  ✓ ws-cor connectivity included")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='PROD Fabric Topology Generator',
...
Examples:
  ...
  python3 prod_fabric_generator.py --site nrt12-12 --fabric ws-c1
    ...
    parser.add_argument('--fabric', required=True,
                       help='Fabric type (e.g., ws-c1)')
    
    args = parser.parse_args()
    
    # Create and run agent
    agent = ProdFabricTopologyGenerator(args.site, args.fabric)
    success = agent.run()
